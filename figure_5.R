...
n.heat <- seq(11, 60)  # generate sample size values
rv.heat <- sapply(c.heat, function(x) {  # calculate formula replication value for all combinations of input parameters
  repVal(c = x, y = y.heat, n = n.heat)
})
df.heat <- expand.grid("C" = c.heat, "Y" = y.heat, "n" = n.heat)  # Make dataframe for heatmap
df.heat$rv <- apply(df.heat, 1, function(x) { # Compute RV for all cases in dataframe
  repVal(c = x["C"], y = x["Y"], n = x["n"])
})


fig5 <- ggplot(df.heat, aes(x = n, y = C/Y, z = rv)) +  # Plot figure 5
